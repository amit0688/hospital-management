...
function DocAppointment({ appointment }) {
        ...
        <div className='bg-white rounded-lg max-w-[400px] h-auto p-4 shadow-lg border'>
            ...
            <div className='flex gap-6 py-4 border-b'>
                <div className='h-[100px] w-[100px] rounded-lg'>
                    <img src={appointment?.user?.avatar ? appointment.user.avatar : defaultPic} className='h-[100px] w-[100px] rounded-lg' alt="User's Avatar" />
                </div>
                <div className='flex flex-col gap-1 mt-2'>
                    <h1 className='font-[600]'>{appointment?.user?.fullname}</h1>
                    {/* <div className='flex items-center gap-1'>
                        <FaLocationDot />
                        <h1 className='text-xs mt-2 md:leading-5 line-clamp-3'>{appointment?.user?.city}</h1>
                    </div> */}
