...
import { useUpdateDocAvatarMutation } from '@/slices/doctorUserApiSlice';





function UpdateDocPic() {

  const { userInfo } = useSelector((state) => state.auth)
  const [avatar, setAvatar] = useState("")
//   const [images, setImages] = useState([])

  const [updateAvatar, { isLoading }] = useUpdateDocAvatarMutation();
            ...
            <div className='w-[80%]  max-w-[400px]'>
              ...
              <div className='bg-slate-100 p-4 rounded-lg'>
                <form action="" onSubmit={handleSubmitAvatar}>
                  <div className='flex gap-5 items-center justify-between w-full'>
                    {avatar && (
                      <div>
                        <img src={avatar instanceof File ? URL.createObjectURL(avatar) : userInfo.avatar} className='w-14 h-12 object-cover  rounded-full ' alt="Selected" />
                      </div>
                    )
              ...
              {/* <div className='bg-slate-100 p-4 rounded-lg'>
                <form action="" onSubmit={handleSubmitImages}>
                  ...
                  <button type='submit'
                    className='w-[100%] max-w-[400px] py-5 rounded-lg text-headingColor text-md font-[600] bg-slate-200 hover:bg-slate-500/50 flex items-center justify-center mt-3'
                    disabled={isLoading}>{isLoading ? <div className="loader"></div> : "Update Images"}</button>
                </form>
              </div> */}
